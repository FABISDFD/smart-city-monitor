...
const BACKEND_URL = 'https://tu-backend.herokuapp.com'; // ⚠️ CAMBIAR por URL de tu backend
const USE_BACKEND = true; // true = usar backend, false = simulación local
...
async function checkBackendConnection() {
  try {
    const status = await fetchFromBackend('/api/status');
    backendConnected = true;
    dataSource = status.mode || 'backend';
    updateConnectionStatus(`Conectado - ${getSourceLabel(status.mode)}`, 'green');
    return true;
  } catch (error) {
    backendConnected = false;
    dataSource = 'local_simulation';
    updateConnectionStatus('Backend no disponible - Simulación Local', 'yellow');
    return false;
  }
}
...
function getSourceLabel(mode) {
  const labels = {
    'esp32': 'ESP32 Real',
    'simulation': 'Simulación Backend',
    'local_simulation': 'Simulación Local'
  };
...
async function updateData() {
  ...
  if (USE_BACKEND && backendConnected) {
    try {
      data = await fetchFromBackend('/api/data');
      data.timestamp = new Date();
      
      try {
        const freqData = await fetchFromBackend('/api/frequency');
        data.bands = freqData.bands;
      } catch (e) {
        console.warn('No se pudieron obtener frecuencias');
      }
      
    } catch (error) {
      console.warn('Error backend, usando simulación local');
      backendConnected = false;
...
function updatePrediction(data) {
  ...
  if (trend > 2) trendText = '<i class="fas fa-arrow-up trend-up"></i> Aumentando';
  else if (trend < -2) trendText = '<i class="fas fa-arrow-down trend-down"></i> Disminuyendo';
...
function generateReport() {
  const report = 'REPORTE DE MONITOREO ACÚSTICO URBANO\n' +
    ...
    'Fuente: ' + getSourceLabel(dataSource) + '\n\n' +
...
window.addEventListener('load', async function() {
  ...
  if (USE_BACKEND) {
    updateConnectionStatus('Conectando al backend...', 'yellow');
    await checkBackendConnection();
    
    setInterval(async () => {
      if (!backendConnected && USE_BACKEND) {
        await checkBackendConnection();
      }
    }, 30000);
  } else {
    updateConnectionStatus('Modo Simulación Local', 'yellow');
  }
  ...
  setTimeout(() => {
    const msg = backendConnected ? 
      `✓ Sistema activo - ${getSourceLabel(dataSource)}` : 
      '⚠️ Modo simulación local';
    showNotification(msg, 'yellow');
  }, 1500);
});
